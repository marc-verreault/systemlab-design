'''
    ...
    Python(3.7.3), PyQt(5.13.1), NumPy(1.17.2), Matplotlib(3.1.1),
    SciPy(1.3.1)
...
# MV 20.01.r1 17-Dec-2019 Moved to config special file
...
class SystemLabMainApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.action_PrintSceneObjects.triggered.connect(self.view_scene_objects)
        ...
        self.tableWidget3.item(0,1).setForeground(QtGui.QColor(0, 0, 127)) # MV 20.01.r1
        self.tableWidget3.item(0,1).setBackground(QtGui.QColor(255, 255, 255))
        self.tableWidget3.item(1,1).setForeground(QtGui.QColor(0, 0, 127)) # MV 20.01.r1
    ...
    def project_save_as(self):  
        ...
        if not path: #Field is empty (set "save as" path to root path)
    ...
    def open_script_editor(self):
        ...
        script = str(script) + ' ' + str(root_path) + '/syslab_fb_scripts/script_template_v2_20_dec_19.py'
        try:
            os.system(script)
        except:
...
#                                  html = 'New text item')
    ...
    def update_background_color(self, key_index): #MV 20.01.r1 27-Jan-20 (added key_index)
        ...
        if color.isValid():
            proj.setBackgroundBrush(QtGui.QBrush(QtGui.QColor(color)))
            proj.setBackgroundBrush(QtGui.QBrush(QtGui.QColor(color)))
...
class FunctionalBlockDesignView(QtWidgets.QGraphicsRectItem):
    ...
    def mouseDoubleClickEvent(self, QMouseEvent): #QT specific method
        ...
        for item in items:
            if type(item) is FunctionalBlockDesignView:
                break   
        if config_special.open_script_on_left_double_click == True:
        ...
        if t.size > 0 and self.signal_type != 'Disabled':
            ...
            if (self.signal_type == 'Optical' and 
                ...
                if len(signal_data) >= iteration: # Safety check in case simulation has
                    ...
                    text_sig_pwr_total = ("<tr><td>Sig pwr (total - all ch) (W): </b>" + "</td><td>" 
                                + str(format(sig_pwr, '0.6f')) + "</td><td>" + ""  + "</td></tr>")
                    text_sig_pwr_avg = ("<tr><td>Sig pwr (avg - all ch) (W): </b>" + "</td><td>" 
                                + str(format(sig_pwr_avg, '0.6f')) + "</td><td>" + ""  + "</td></tr>")
                    text_noise_pwr_total = ("<tr><td>Noise pwr (total - all ch) (W): </b>" + "</td><td>" 
                                + str(format(noise_pwr, '0.6f')) + "</td><td>" + ""  + "</td></tr>")
                    text_noise_pwr_avg = ("<tr><td>Noise pwr (avg - all ch) (W): </b>" + "</td><td>" 
                                + str(format(noise_pwr_avg, '0.6f')) + "</td><td>" + ""  + "</td></tr>")
                    all_results = (text_opt_channels + text_sig_pwr_total + text_sig_pwr_avg
                                   + text_noise_pwr_total + text_noise_pwr_avg)
                else:
                    all_results = ("<tr><td colspan='3'><i>No data available for current" 
                                + " iteration</i></td></tr>")
...
class RemoveGraphicsItemsAndDataModelsCommand():
    ...
    def undo(self):   
        ...
        #https://stackoverflow.com/questions/7431084/how-to-delete-all-qgraphicsitem-from-qgraphicsscene
        project.clear()
...
class FunctionalBlockGUI(QtWidgets.QDialog, Ui_FBWindow):
    ...
    def load_parameters_table(self):
        ...
        if length_parameters_list >= self.parametersTable.rowCount(): 
            ...
            self.parametersTable.setRowCount(30)
        ...
        for row in range(0, length_parameters_list):
            ...
            if len(parameters_list[row]) > 4: #Temporary 
                if parameters_list[row][4] == False:
                    ...
                    #self.parametersTable.item(row, 0).setToolTip(parameterName) Future feature
                    parameterValue = parameters_list[row][1]
                    parameterUnit = parameters_list[row][2]
                    parameterNotes = parameters_list[row][3]
