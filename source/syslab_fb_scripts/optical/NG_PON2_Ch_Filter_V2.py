...
import numpy as np
import config
import systemlab_viewers_ng_pon2 as view
...
def run(input_signal_data, parameters_input, settings):
    ...
    passband = float(parameters_input[9][1])*1e9 # GHz -> Hz
    display_filter_profile = int(parameters_input[10][1])
    ...
    noise_field_output = noise_field_rcv
    ...
    for ch in range(0, channels):
        ...
        if opt_channels[0][3].ndim == 2: # Polarization format: Ex-Ey
            # PORT 1
            Y_x_1 = np.fft.ifftshift(Y_x_1)
            opt_field_out_1[ch, 0] = np.fft.ifft(Y_x_1)
            Y_y_1 = np.fft.ifftshift(Y_y_1)
            opt_field_out_1[ch, 1] = np.fft.ifft(Y_y_1)
        else:
            # PORT 1
            Y_1 = np.fft.ifftshift(Y_1)
            opt_field_out_1[ch] = np.fft.ifft(Y_1)
    ...
    # Isolation analysis------------------------------------------------------------
    ctr_freq_2 = ctr_freq_1 - 100e9 #100 GHz spacing is assumed
    tr_ch1 = np.interp(ctr_freq_1, freq_array, tr_fcn_filt_1)
    p1_sig_pwr = np.square(np.abs(tr_ch1))
    p1_adj_leakage = np.interp(ctr_freq_2 + passband/2, freq_array, tr_fcn_filt_1)
    p1_adj_pwr = np.square(np.abs(p1_adj_leakage))
    adj_isolation_ratio = 10*np.log10(p1_adj_pwr/p1_sig_pwr)
        
        # Create plot instance
    if display_filter_profile == 2:
        project.p1_trans_freq = view.Demux_Analyzer('Filter transfer (all ports)', freq_array, 'Frequency (Hz)', 
                                                                         total_profile, 'Transmission', wave_freq, freq_start, 
                                                                         freq_end, ctr_freqs, passband/2, graph_units)
        project.p1_trans_freq.show()
    ...
    #bw_isolation_result = ['Passband (isolation)', bw_isolation, 'GHz', ' ', False, '0.2f']
    adj_isolation_result = ['Adjacent channel isolation', adj_isolation_ratio, 'dB', ' ', False, '0.2f']
    ...
    optical_channels_1 = []
    for ch in range(0, channels):
        opt_ch_1 = [int(wave_key[ch]), wave_freq[ch], jones_vector[ch], opt_field_out_1[ch], noise_field_output[ch, :]]
