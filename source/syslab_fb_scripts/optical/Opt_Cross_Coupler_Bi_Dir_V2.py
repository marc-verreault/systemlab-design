...
def run(input_signal_data, parameters_input, settings):
    ...
    if opt_channels[0][3].ndim == 2: # Polarization format: Ex-Ey
        e_field_input_P1 = np.full([2, n], 0 + 1j*0, dtype=complex)
        e_field_input_P2 = np.full([2, n], 0 + 1j*0, dtype=complex)
    else:
        e_field_input_P1 = np.full(n, 0 + 1j*0, dtype=complex)
    ...
    if opt_channels[0][3].ndim == 2: # Polarization format: Ex-Ey
        e_field_output_P3  = np.full([2, n], 0 + 1j*0, dtype=complex)
        e_field_output_P4  = np.full([2, n], 0 + 1j*0, dtype=complex)
    else:
        e_field_output_P3  = np.full(n, 0 + 1j*0, dtype=complex)
        e_field_output_P4  = np.full(n, 0 + 1j*0, dtype=complex)
    ...
    if segment == 1 or feedback_mode == 0:
        ...
        if opt_channels[0][3].ndim == 2: # Polarization format: Ex-Ey
            fdk.e_field_output_P2 = np.full([2, n], 0 + 1j*0, dtype=complex)
            e_field_input_P3 = np.full([2, n], 0 + 1j*0, dtype=complex)
        else:
    ...
    if feedback_mode == 2:
        ...
        for seg in range(start_index, n):
            if opt_channels[0][3].ndim == 2: # Polarization format: Ex-Ey
                s21_x_E3In_x = e_field_input_P3[0, seg - int(segment_length)] * (1j*np.sqrt(cc))
                fdk.e_field_output_P2[0, seg] = s21_x_E3In_x
                s21_x_E3In_y = e_field_input_P3[1, seg - int(segment_length)] * (1j*np.sqrt(cc))
                fdk.e_field_output_P2[1, seg] = s21_x_E3In_y
            else:
