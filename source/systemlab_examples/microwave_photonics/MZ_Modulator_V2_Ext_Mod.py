...
Version 2.0 (19.02.r2 26 Sep 2019)
...
def run(input_signal_data, parameters_input, settings):
    ...
    v_max_swing_in = np.max(v1) - np.min(v2)
    mod_index = (v_max_swing_in/v_pi) * (np.pi/2)
    ...
    if disp_trans_fcn == 2: #Based on Fig 5-15 of Ref 1
        ...
        config.mzm_graph = config.view.MZMAnalyzer(v, e_field, e_field_at_bias, v_op_point)
    ...
    MZM_results = [v1_bias_result, v2_bias_result, mz_op_voltage_result]
    
    
    #Send update to data box (data_table_opt_im_1)
    config.data_tables['microwave_em_1'] = []
    data_1 = ['V_pi', v_pi, '0.2f', ' ']
    data_2 = ['Operating point (V/Vpi)', v_op_point, '0.2f', ' ']
    data_3 = ['Max voltage swing (driver)', v_max_swing_in, '0.2f', ' ']
    data_4 = ['Modulation index', mod_index, '0.2f', ' ']
    data_list = [data_1, data_2, data_3, data_4]
    config.data_tables['microwave_em_1'].extend(data_list)
