...
def run(input_signal_data, parameters_input, settings):
    ...
    if pwr_units == 'dBm':
        optical_pwr = optical_pwr + iteration*0.5 # Add dB per iteration
        optical_pwr = np.power(10, optical_pwr/10)
    line_width = float(parameters_input[6][1]) #Laser linewidth (MHz)
    phase_deg = float(parameters_input[7][1]) #Optical phase (deg)
    # Relative intensity noise (header)
    ref_bw = float(parameters_input[9][1]) #Reference bandwidth (Hz)
    rin = float(parameters_input[10][1]) #RIN (dB/Hz)
    include_rin = int(parameters_input[11][1]) #Include RIN in model
    ...
    include_ase = int(float(parameters_input[22][1]))
    add_ase_to_signal = int(float(parameters_input[23][1]))
