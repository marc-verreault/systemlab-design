...
def run(input_signal_data, parameters_input, settings):
    ...
    for ch in range(0, channels):
        ...
        config.data_tables['opt_meas'].append(carrier_data_panel)
        ...
        if display_x_y == 2: 
            ...
            data_list = [data_1, data_2, data_3]
            config.data_tables['opt_meas'] .extend(data_list)
        # X-polarization data
        if display_x == 2: 
    ...
    '''==PLOTTING========================================='''
    if iteration == 1: #Set axes lists to zero
        project.photons_per_bit = []
        project.ser_th = []
        project.ber_th = []
    project.photons_per_bit.append(photons_per_bit)
    
    ser_th = 0.5*special.erfc(np.sqrt(photons_per_bit/2))  
    ber_th = 0.5*special.erfc(np.sqrt(photons_per_bit))  
    project.ser_th.append(ser_th)
    project.ber_th.append(ber_th)
