"""
Optical Cross Coupler
"""
import numpy as np
import config
import project_michelson as fdk
...
def run(input_signal_data, parameters_input, settings):
    ...
    if feedback_mode == 2:
        ...
        for seg in range(start_index, n):
            ...
            s22_x_E4In = e_field_input_P4[seg - int(segment_length)] * (np.sqrt(1-cc))
            fdk.e_field_output_P2[seg] = s21_x_E3In + s22_x_E4In
    else:
        # Feedback mode is OFF - calculate output field at P2 for all samples
        s21_x_E3In = e_field_input_P3 * (1j*np.sqrt(cc))
        s22_x_E4In = e_field_input_P4 * (np.sqrt(1-cc))
        fdk.e_field_output_P2 = s21_x_E3In + s22_x_E4In
